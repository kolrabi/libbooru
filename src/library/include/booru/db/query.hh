...
{
  public:
    Where(StringView const& _A, StringView const& _B, StringView const& _Op)
        : A{_A}, B{_B}, Op{_Op}
    {
    }

    template <class TA, class TB> static Where Equal(TA const& _A, TB const& _B)
    {
        return Where(String(_A), String(_B), "=="sv);
    }

    template <class TA, class TB> static Where In(TA const& _A, TB const& _B)
    {
    ...
    {
        return Strings::Join(StringVector{"( ", A, ") ", Op, " ( ", B, " )"},
                             "");
    }
...
template <class Derived> class Query
{
  public:
    ...
    // Add a key where condition (WHERE key = $key). Bind "$key" in the prepared
    // statement.
...
// Simple delete query. Deletes rows that match keys and values. Will not delete
// all rows if no key is given.
class Delete : public Query<Delete>
{
  ...
  protected:
    String AsString() const override
    {
        ...
        {
            LOG_ERROR(
                "Refusing to create a DELETE statement without a where "
                "clause!");
...
template <class TValue> class InsertEntity : public Insert
{
  public:
...
template <class TValue> class UpdateEntity : public Update
{
  public:
