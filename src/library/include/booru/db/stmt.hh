...
class IStmt : public std::enable_shared_from_this<IStmt>
{
    static constexpr auto LOGGER = "booru.db.stmt";
  ...
  public:
    ...
    virtual ExpectedStmt BindValue(StringView const& _Name,
                                   ByteSpan const& _Blob)  = 0;
    virtual ExpectedStmt BindValue(StringView const& _Name,
                                   FLOAT const& _Value)    = 0;
    virtual ExpectedStmt BindValue(StringView const& _Name,
                                   INTEGER const& _Value)  = 0;
    virtual ExpectedStmt BindValue(StringView const& _Name,
                                   TEXT const& _Value)     = 0;
    ...
    ExpectedStmt BindValue(StringView const& _Name,
                           BLOB<BlobSize> const& _Value);

    template <class TValue>
    ExpectedStmt BindValue(StringView const& _Name,
                           NULLABLE<TValue> const& _Value);

    template <class TValue> static auto BindValueFn()
    {
        return static_cast<ExpectedStmt (IStmt::*)(
            StringView const&, TValue const&)>(&IStmt::BindValue);
    }
    ...
    template <class TEntity> ExpectedVector<TEntity> ExecuteList();
};
...
template <size_t BlobSize>
ExpectedStmt IStmt::BindValue(StringView const& _Name,
                              BLOB<BlobSize> const& _Value)
{
...
ExpectedStmt IStmt::BindValue(StringView const& _Name,
                              NULLABLE<TValue> const& _Value)
{
    if (_Value.has_value()) { return BindValue(_Name, _Value.value()); }
    else { return BindNull(_Name); }
}

// GetColumnValue

template <size_t BlobSize>
ResultCode IStmt::GetColumnValue(int _Index, BLOB<BlobSize>& _Value)
{
    ...
    {
        LOG_WARNING(
            "Data of size {} got truncated trying to store in blob of size {}",
            blob.size(), BlobSize);
    ...
    {
        LOG_WARNING(
            "Data of size {} got padded with zeroes trying to store in "
            "blob of size {}",
            blob.size(), BlobSize);
    }
...
template <class TValue>
ResultCode IStmt::GetColumnValue(int _Index, NULLABLE<TValue>& _Value)
{
...
template <class TValue>
ResultCode IStmt::GetColumnValue(StringView const& _Name, TValue& _Value)
{
...
template <class TValue> Expected<TValue> IStmt::ExecuteScalar(bool _NeedRow)
{
    TValue value;
...
template <class TEntity> ExpectedVector<TEntity> IStmt::ExecuteList()
{
