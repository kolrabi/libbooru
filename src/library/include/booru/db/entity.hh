...
#define ENTITY_PROPERTY_KEY(Name) \
    ...
    ExpectedVector<DB::INTEGER> CollectIds(Vector<TEntity> const &_Entities)
    {
        ...
        std::ranges::transform(_Entities, std::begin(Ids), [](auto const &e)
                               { return e.Id; });
        return Ids;
    }
    ...
    template <class TEntity>
    ExpectedVector<TEntity> GetAll(DBPtr _DB)
    {
    ...
    template <class TEntity, class TValue>
    ExpectedVector<TEntity> GetAllWithKey(DBPtr _DB, StringView const &_KeyColumn, TValue const &_KeyValue)
    {
        CHECK_VAR_RETURN_RESULT_ON_ERROR(stmt, DB::Query::Select(TEntity::Table).Key(_KeyColumn).Prepare(_DB));
        CHECK_RETURN_RESULT_ON_ERROR(stmt.Value->BindValue(_KeyColumn, _KeyValue));
        return stmt.Value->ExecuteList<TEntity>();
    }
