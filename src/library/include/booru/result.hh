...
/// @brief Data structure representing an expected value and a possibly
/// unexpected result/error code.
/// @tparam TValue Type of expected data value.
template <class TValue> struct [[nodiscard]] Expected
{
    ResultCode Code;
    TValue Value;
    ...
    /// @brief Successful value constructor. Result code and value will be
    /// moved.
    /// @param _Value Value to set.
    /// @param _Code Result code to set, defaults to OK.
    Expected(TValue&& _Value, ResultCode _Code = ResultCode::OK)
        : Code{_Code}, Value{std::move(_Value)}
    {
    }
    Expected(TValue const& _Value, ResultCode _Code = ResultCode::OK)
    ...
    auto Then(Callable _Callable, TArgs... args)
        -> decltype(std::invoke(_Callable, Value, args...))
    {
        if (ResultIsError(Code)) return {Code};
        return std::invoke(_Callable, std::move(Value), args...);
    }

    template <class Callable, class TObj, typename... TArgs>
    auto Then(Callable _Callable, TObj& _Obj, TArgs... args)
        -> decltype(std::invoke(_Callable, _Obj, Value, args...))
    {
        if (ResultIsError(Code)) return {Code};
        return std::invoke(_Callable, _Obj, std::move(Value), args...);
    }

    template <class TValue2> Expected<TValue2> ThenValue(TValue2& _Value2)
    {
        return Expected<TValue2>{_Value2, Code};
    }

    Expected OnError(auto _Callable)
    {
        if (ResultIsError(Code)) _Callable(Code);
    ...
    Expected Update(Expected& _Expected)
    {
        _Expected = *this;
        return std::move(*this);
    }
    ...
    static inline Expected<TValue> ErrorOrObject(ResultCode _Code,
                                                 TValue&& _Object)
    {
        if (ResultIsError(_Code)) { return _Code; }
        return _Object;
    }
};

template <class TValue> using ExpectedOwning = Expected<Owning<TValue>>;
template <class TValue> using ExpectedShared = Expected<Shared<TValue>>;
template <class TValue> using ExpectedVector = Expected<std::vector<TValue>>;
...
static inline bool
CheckResult(String const& _LoggerName, TValue const& _Result,
            StringView const& _ResultStr, std::source_location const _Location,
            StringView const& _Msg = "", TArgs const&... args)
{
    if (_LoggerName.empty()) return ResultIsError(_Result);

    auto logger = log4cxx::Logger::getLogger(String(_LoggerName));
    if (ResultIsError(_Result))
    {
        LOG4CXX_ERROR_FMT(logger, "{}:{}:{}: {} failed: {}",
                          _Location.file_name(), _Location.line(),
                          _Location.function_name(), _ResultStr,
                          ResultToString(_Result));
        if (!_Msg.empty())
            LOG4CXX_ERROR_FMT(
                logger, "{}",
                std::vformat(_Msg, std::make_format_args(args...)));
        return true;
    }
    LOG4CXX_TRACE_FMT(logger, "{}:{}:{}: {} OK ({})!", _Location.file_name(),
                      _Location.line(), _Location.function_name(), _ResultStr,
                      ResultToString(_Result));
    if (!_Msg.empty())
        LOG4CXX_TRACE_FMT(logger, "{}",
                          std::vformat(_Msg, std::make_format_args(args...)));
    return false;
}
...
#define CHECK_RETURN_RESULT_ON_ERROR(x, ...)                                   \
    {                                                                          \
        auto const& _result = (x);                                             \
        if (CheckResult(LOGGER, _result, #x,                                   \
                        std::source_location::current() __VA_OPT__(, )         \
                            __VA_ARGS__))                                      \
...
#define CHECK_ASSERT(x)                                                        \
    {                                                                          \
        LOG4CXX_ASSERT_FMT(log4cxx::Logger::getLogger(LOGGER), x,              \
                           "Assertion failed: {}", #x);                        \
    }
