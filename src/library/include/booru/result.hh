...
namespace Booru
{
    ...
    struct [[nodiscard]] Expected
    {
        ...
        Expected(TValue const &_Value, ResultCode _Code = ResultCode::OK) : Code{_Code}, Value{_Value} {}
...
#define CHECK(x, ...)                                                                                 \
    {                                                                                                 \
        ...
        CheckResult(LOGGER, _result, #x, std::source_location::current() __VA_OPT__(, ) __VA_ARGS__); \
    }
...
#define CHECK_RETURN_RESULT_ON_ERROR(x, ...)                                                              \
    {                                                                                                     \
        auto const &_result = (x);                                                                        \
        if (CheckResult(LOGGER, _result, #x, std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)) \
