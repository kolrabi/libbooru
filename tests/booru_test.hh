...
#define TEST_CHECK_EQUAL(cond, v)                                              \
    {                                                                          \
        test_check((cond), #cond);                                             \
        test_equal((cond).Value, (v), #cond, #v);                              \
    }

#define TEST_EQUAL(cond, v)                                                    \
    {                                                                          \
        test_equal((cond), (v), #cond, #v);                                    \
    }
#define TEST_TRUE(cond)                                                        \
    {                                                                          \
        test_equal(!!(cond), true, #cond);                                     \
    }
...
static inline void PASS() { exit(EXIT_SUCCESS); }

static inline void FAIL() { exit(EXIT_FAILURE); }
...
static inline void test_equal(T a, U b, char const* _CondA, char const* _CondB)
{
    using namespace Booru;
    auto aStr = ToString(a);
    auto bStr = ToString(b);
    if (!(a == b))
    {
